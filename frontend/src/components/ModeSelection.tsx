...
export const ModeSelection: React.FC<ModeSelectionProps> = ({ onSelectMode }) => {
    return (
        <div className="max-w-4xl mx-auto mt-20 animate-fade-in">
            <h2 className="text-3xl font-bold text-center text-gray-800 dark:text-gray-100 mb-12">Choose Your Workflow</h2>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {/* Upload Mode Card */}
                <button
                    onClick={() => onSelectMode('upload')}
                    className="glass-panel p-8 text-left group hover:scale-[1.02] active:scale-95 transition-all duration-300 flex flex-col items-start dark:hover:bg-white/10"
                >
                    <div className="w-14 h-14 bg-blue-50 dark:bg-white/10 rounded-2xl flex items-center justify-center mb-6 text-blue-600 dark:text-blue-400 group-hover:scale-110 transition-transform duration-300">
                        <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" />
                        </svg>
                    </div>
                    <div className="relative z-10 w-full">
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Upload Excel</h3>
                ...
                <button
                    ...
                    className="glass-panel p-8 text-left group hover:scale-[1.02] active:scale-95 transition-all duration-300 flex flex-col items-start dark:hover:bg-white/10"
                >
                    <div className="w-14 h-14 bg-purple-50 dark:bg-white/10 rounded-2xl flex items-center justify-center mb-6 text-purple-600 dark:text-purple-400 group-hover:scale-110 transition-transform duration-300">
                        <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </div>
                    <div className="relative z-10 w-full">
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Manual Entry</h3>
                        <p className="text-gray-500 dark:text-gray-400 leading-relaxed">
